...
void GWindow::drawLineTo(int x, int y) {
    drawLineTo(I2Point(x, y));
}
...
void GWindow::drawLineRel(const I2Vector& v) {
    drawLineTo(m_ICurPos + v);
}

void GWindow::drawLineRel(const R2Vector& v) {
    drawLineTo(m_RCurPos + v);
}

void GWindow::drawLineRel(int dx, int dy) {
    drawLineRel(I2Vector(dx, dy));
}
...
void GWindow::fillPolygon(const I2Point* points, int numPoints) {
    if (numPoints <= 2)
        return;
...
void GWindow::fillPolygon(const R2Point* points, int numPoints) {
    if (numPoints <= 2)
        return;
